...
```sh
npm run test:games    # deterministic runtime and Jev-gate checks; no model or deploy
npm run eval:games    # live game-generation prompts; run npm run dev first
```

`eval:games` scores the selected generated surface and playable contract rather than
...
**Node 22.12 or newer** (developed on 26) and **npm**. The lockfile is
...
`wrangler.jsonc` keeps `LLM_PROFILE` at `dev`; configure `DEV_LLM_*` to use
your development provider. Without it, local runs fall back to OpenAI. The
deploy script selects the OpenAI-backed `demo` profile for production.
