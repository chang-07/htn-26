...
The dashboard loads `/api/events`; `/api/events/:id` also checks membership. Membership comes from actual phone handles recorded by the agent, never browser-supplied plan IDs or web voters. The agent also reads the active Linq chat roster (cached for five minutes) so people who haven’t texted yet can see their plans. During a Linq outage, recorded participants and the last known roster remain usable. Membership is retained for event participants; removal from an iMessage group does not revoke historical dashboard access.
