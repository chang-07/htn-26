...
function SignIn({ onSignedIn }: { onSignedIn: () => void }) {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      else { await api("/api/account/verify", { method: "POST", body: JSON.stringify({ id: challenge, code }) }); onSignedIn(); }
  ...
  </form><p className="account-footnote">{challenge ? "Your code expires in 10 minutes." : "New here? Your account is created when you verify your number."}</p></main></div>;
}
export function AccountDashboard() {
  ...
  if (signedOut) return <SignIn onSignedIn={() => setAttempt(n => n + 1)} />;
  ...
  return <Dashboard key={loaded.account.id} initialData={loaded.data} initialRevision={loaded.revision} account={loaded.account} onSignOut={() => { setLoaded(null); setSignedOut(true); }} />;
}
