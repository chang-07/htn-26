...
export function RunOverview({ run, events, onPick }: { run: RunSummary; events: DiagnosticEvent[]; onPick: (seq: number) => void }) {
  ...
  const error = diagnostics.errors.at(-1);
  ...
  return <section className={`rv-overview is-${status.key}`} aria-label="Run summary">
    ...
    {error && <button className="rv-error-link" onClick={() => onPick(error.seq)}><span><strong>Latest error</strong><span>{operation(error)}</span></span><span>Inspect error →</span></button>}
