...
export default function FlipCard({
  ...
  const onPointerDown = e => {
    if (disabled || e.button !== 0 || grip.current || e.target.closest('a, button, input, select, textarea, [data-no-flip]')) return;
    try {
  ...
  const onPointerMove = e => {
    ...
    if (g && g.id === e.pointerId) {
      if (Math.abs(e.clientY - g.y) > g.slop && !draggable) g.scrolled = true;
  ...
  const release = (e, cancelled) => {
    ...
    if (!g.moved) {
      if (!cancelled && !g.scrolled && flipOnClick) flip(false);
  ...
  const onKeyDown = e => {
    if (e.target !== e.currentTarget || disabled || (e.key !== 'Enter' && e.key !== ' ')) return;
    e.preventDefault();
    if (!e.repeat) flip(true);
  };
  const onClick = e => {
    if (!disabled && e.detail === 0 && !e.target.closest('a, button, input, select, textarea, [data-no-flip]')) flip(true);
  };
  ...
  return (
    <div
      ref={rootRef}
      role={interactiveContent ? "group" : "button"}
      tabIndex={disabled || interactiveContent ? -1 : 0}
      aria-pressed={interactiveContent ? undefined : shown}
